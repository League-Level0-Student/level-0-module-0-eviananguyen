...
laneWidth = height / numberOfTurkeys;
 ...
 dunn = new Turkey(0, laneWidth* 0);
 nicholas = new Turkey(0, laneWidth *1);
 evangelina =new Turkey (0, laneWidth*2);
 eviana = new Turkey (0, laneWidth*3);
 turkey = new Turkey (0, laneWidth*4);
}
...
void checkForWinner() {
 ...
 if(dunn.x > width){
   gameOver=true;
   text("winner winner chicken dinner",20, laneWidth*1);
 }
   
if(nicholas.x > width){
  gameOver=true;
  text("winner winner chicken dinner", 20, laneWidth*2);
}
 if(evangelina.x > width){
  gameOver=true;
  text("winner winner chicken dinner", 20, laneWidth*3);
}
if(eviana.x > width){
  gameOver=true;
  text("winner winner chicken dinner", 20, laneWidth*4);
}
if(turkey.x > width){
  gameOver=true;
  text("winner winner chicken dinner", 20, laneWidth*5);
}
}
